...
interface MarqueeOptionsBarProps {
  onCancel: () => void;
  hasSelection: boolean;
  onAction: (action: 'copy' | 'cut' | 'duplicate' | 'erase') => void;
}

export const MarqueeOptionsBar: React.FC<MarqueeOptionsBarProps> = ({
  onCancel,
  hasSelection,
  onAction
}) => {
  ...
  return (
    <div className="h-12 bg-[#191919] border-b border-zinc-800 shadow-sm shrink-0 flex items-center px-4 gap-6 z-[90]">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <X size={14} />
          <span className="text-[10px] font-bold uppercase">{t('common.cancel', 'Cancelar')}</span>
        </button>
      </div>
      ...
      <div className="text-[10px] text-zinc-500 italic">
        {t('editor.marquee.hint', 'Clique e arraste para definir uma área retangular de seleção.')}
      </div>
    </div>
  );
};
